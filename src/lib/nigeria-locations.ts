/** In-process Nigeria catalog — served by /api/locations without hitting the database. */
...
/** LGAs per state (official local government areas). */
...
export function resolveNigeriaStateName(state: string): string {
  ...
  return NIGERIA_STATES.find((name) => name.toLowerCase() === raw.toLowerCase()) ?? raw;
}

export function citiesForState(state: string): string[] {
  const key = resolveNigeriaStateName(state);
  return NIGERIA_CITIES_BY_STATE[key] ?? [];
}
