...
export const createShortletReservationSchema = z.object({
  ...
  if (!hasUnit && !hasProperty) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["propertyId"],
      message: "Select a location, or assign an apartment now — you can also assign the apartment at check-in.",
    });
  }
...
export const createShortletBookingBatchSchema = z.object({
