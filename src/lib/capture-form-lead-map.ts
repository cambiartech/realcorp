...
export function resolveLeadNameFromValues(fields: CaptureFormField[], values: Record<string, string>): string | null {
...
export function resolveLeadEmailFromValues(fields: CaptureFormField[], values: Record<string, string>): string | null {
  const emailField = fields.find((f) => f.type === "email");
  if (emailField && values[emailField.key]?.trim()) return values[emailField.key].trim();
  return values.email?.trim() || null;
}

export function resolveLeadPhoneFromValues(fields: CaptureFormField[], values: Record<string, string>): string | null {
...
export function buildLeadNotesFromCaptureValues(fields: CaptureFormField[], values: Record<string, string>): string | null {
  const lines: string[] = [];
