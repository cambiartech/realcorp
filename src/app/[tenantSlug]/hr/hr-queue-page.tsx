...
export default async function HrQueuePage({
  ...
  const jobRoles = mergeOrgJobRoles([
    ...((tenant.settings?.orgJobRoles as string[] | null | undefined) ?? []),
    ...profiles.map((p) => p.position?.trim() || ""),
  ]).sort((a, b) => a.localeCompare(b));
      ...
      payslipRuns={payslipRuns.map((r) => ({
        ...
        payslips: r.payslips.map((s) => {
          return {
            ...
            location: s.profile.addressState || s.profile.department || "",
            hireDate: s.profile.dateOfJoining ? s.profile.dateOfJoining.toISOString().slice(0, 10) : "",
