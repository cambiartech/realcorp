...
const STATUS_STYLES: Record<string, string> = {
  VACANT_CLEAN: "border-emerald-500/40 bg-emerald-500/10",
  VACANT_DIRTY: "border-amber-500/40 bg-amber-500/10",
  OCCUPIED: "border-blue-500/40 bg-blue-500/10",
        ...
        {rooms.map((room) => (
          <article
            ...
            className={[
              ...
              room.alertLevel === "overdue" ? "ring-2 ring-red-500" : room.alertLevel === "due-soon" ? "ring-2 ring-amber-500" : "",
            ].join(" ")}
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-muted">
                  {room.propertyName ? `${room.propertyName} · ` : ""}{room.location}
                </p>
              </div>
            ...
            {room.checkoutLabel ? (
              <p className={["mt-1 text-xs", room.alertLevel === "overdue" ? "font-semibold text-red-600" : "text-muted"].join(" ")}>
            ...
            {canHousekeeping && room.statusValue !== "OCCUPIED" ? (
              <div className="mt-3 flex flex-wrap gap-1">
                {room.statusValue !== "VACANT_CLEAN" ? (
                  <ActionBtn disabled={isPending} onClick={() => setStatus(room.id, "VACANT_CLEAN")}>Mark clean</ActionBtn>
                ) : null}
                {room.statusValue !== "VACANT_DIRTY" ? (
                  <ActionBtn disabled={isPending} onClick={() => setStatus(room.id, "VACANT_DIRTY")}>Mark dirty</ActionBtn>
                ) : null}
                {room.statusValue !== "OUT_OF_ORDER" ? (
                  <ActionBtn disabled={isPending} onClick={() => setStatus(room.id, "OUT_OF_ORDER")}>Out of order</ActionBtn>
                ) : null}
              </div>
            ) : null}
          </article>
