...
export function ReservationsWorkspace({
  ...
  const newBookingHref = (checkIn?: string) => {
    ...
    return checkIn ? `${base}?checkIn=${encodeURIComponent(checkIn)}&checkOut=${encodeURIComponent(checkIn)}` : base;
  };
  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        <KpiCard label="Total" value={kpis.total} active={statusFilter === "ALL"} onClick={() => setStatusFilter("ALL")} />
        <KpiCard label="Pending" value={kpis.pending} active={statusFilter === "PENDING"} onClick={() => setStatusFilter("PENDING")} />
        <KpiCard label="Confirmed" value={kpis.confirmed} active={statusFilter === "CONFIRMED"} onClick={() => setStatusFilter("CONFIRMED")} />
        <KpiCard label="Checked in" value={kpis.checkedIn} active={statusFilter === "CHECKED_IN"} onClick={() => setStatusFilter("CHECKED_IN")} />
        <KpiCard label="Checked out" value={kpis.checkedOut} active={statusFilter === "CHECKED_OUT"} onClick={() => setStatusFilter("CHECKED_OUT")} />
        <KpiCard label="No shows" value={kpis.noShow} active={statusFilter === "NO_SHOW"} onClick={() => setStatusFilter("NO_SHOW")} />
      </section>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {view === "list" ? (
            <DataExportMenu
              ...
              headers={["Guest", "Booking", "Source", "Apartment", "Stay", "Nights", "Total", "Balance", "Status"]}
              keys={["guest", "booking", "source", "apartment", "stay", "nights", "total", "balance", "status"]}
              rows={exportRows}
            />
          ) : null}
          {canManage ? (
            <Link href={newBookingHref()} className="rounded-md border border-foreground bg-foreground px-4 py-2 text-sm font-semibold text-background">
              New booking
            </Link>
          ) : null}
        </div>
      ...
      {view === "calendar" ? (
        ...
        <div className="rounded-lg border border-foreground/10 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted">Create your first booking — apartment assignment is optional until check-in.</p>
          {canManage ? (
            <Link href={newBookingHref()} className="mt-4 inline-block rounded-md border border-foreground bg-foreground px-4 py-2 text-sm font-semibold text-background">
              Create first booking
            </Link>
          ) : null}
        </div>
      ) : visibleReservations.length === 0 ? (
        <p className="rounded-lg border border-foreground/10 p-6 text-sm text-muted">No reservations match your filters.</p>
      ) : (
            ...
            {visibleReservations.map((r) => (
              <tr key={r.id} className="border-t border-foreground/10">
                ...
                <td className="px-4 py-3">
                  {r.hasApartment ? r.unitName : <span className="text-amber-700">{r.unitName}</span>}
                </td>
                ...
                {canManage ? (
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {!r.hasApartment ? (
                        <ActionBtn disabled={isPending} onClick={() => { setAssignOpen(r.id); setAssignUnitId(unitOptions[0]?.id || ""); }}>
                          Assign apartment
                        </ActionBtn>
                      ) : null}
                      {r.statusValue === "PENDING" ? (
                        <ActionBtn disabled={isPending} onClick={() => run(() => updateShortletReservationStatus(tenantSlug, r.id, "CONFIRMED"), "Confirmed.")}>
                          Confirm
                        </ActionBtn>
                      ) : null}
                      ...
                      {isPreArrivalStatus(r.statusValue) || r.statusValue === "CHECKED_IN" ? (
                        <>
                          ...
                          {isPreArrivalStatus(r.statusValue) ? (
                            <ActionBtn disabled={isPending} onClick={() => run(() => updateShortletReservationStatus(tenantSlug, r.id, "NO_SHOW"), "Marked no-show.")}>
                              No show
                            </ActionBtn>
                          ) : null}
      ...
      {assignOpen ? (
        <ModalOverlay open={Boolean(assignOpen)} onClose={() => setAssignOpen(null)} panelClassName={MODAL_PANEL_LG}>
          <h2 className="text-lg font-bold">Assign apartment</h2>
          <p className="mt-1 text-sm text-muted">Choose a short-let apartment for this booking. Pricing will be calculated from the apartment rates.</p>
          <form
            className="mt-4 space-y-3"
            onSubmit={(e) => {
              e.preventDefault();
              run(
                () => assignShortletReservationApartment(tenantSlug, { reservationId: assignOpen, unitId: assignUnitId }),
                "Apartment assigned.",
              );
              setAssignOpen(null);
            }}
          >
            <label className="block text-sm text-muted">
              Apartment
              <UiSelect className="mt-1" value={assignUnitId} onChange={(e) => setAssignUnitId(e.target.value)} required>
                {unitOptions.map((o) => (
            ...
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setAssignOpen(null)} className="rounded-md border px-3 py-2 text-sm">Cancel</button>
              <button type="submit" disabled={isPending || !assignUnitId} className="rounded-md bg-foreground px-3 py-2 text-sm font-semibold text-background">Assign</button>
            </div>
          </form>
        </ModalOverlay>
      ) : null}
